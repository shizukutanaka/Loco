...
2. **7 つすべてが .NET 専用**。フロントエンドの **630 テストが CI に一切乗っていない** —
...
- **backend テストはローカルで実行できます**。`scripts/run-tests-offline.sh` が
  ...
  ＋リフレクションによるランナー）を使って **432 件全てを実行**します。
  ...
  **実パッケージに対する本物のビルドとテストは、PR #9 で緑になりました。**
  既存の `ci.yml` の `build-and-test` ジョブが ubuntu / macOS / Windows の
  3 プラットフォームで `dotnet restore` → `build` → `test` を実行し、すべて成功
  しています。したがって「実パッケージ未検証」という但し書きはもう不要です。
  ハーネス側は依然として `AddJwtBearer` 配線が自前で Swashbuckle スタブも
  不活性なので、ローカルの緑は CI の代わりにはなりません — 逆に、CI が
  その役目を果たしています。

  なお初回の CI では **7 件のコンパイルエラー**が出ました。すべてテスト
  プロジェクト側で、いずれも**ハーネスが実物より寛容だった**ために
  ローカルで気付けなかったものです（`Loco.Cli.Tests` の FluentAssertions
  参照漏れ、実物に存在しない `BeOneOf` オーバーロード、`WithMessage` の
  部分一致 vs ワイルドカード一致）。修正済みで、ハーネスからも
  実物に無いオーバーロードを削除しました。
