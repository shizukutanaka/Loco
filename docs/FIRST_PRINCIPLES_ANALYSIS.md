...
| # | 過剰 | 規模 | 実態 |
|---|------|------|------|
| **O1** | `src/Loco.Core/AIPlatform/ExternalSecretsEngine.cs`（削除済み） | **1,265 行** | Vault/AWS/Azure/GCP 連携を騙る。中身は `random.Next()` で偽の同期メトリクスを返すだけ。**実際の秘密保存が存在しない一方でこれがある** — 過不足の象徴 |
